 ...
 * To execute the login authentication protocol, both
 * {@link Owl_Server} and {@link Owl_Client} must be constructed. 
 * It is assumed that the registration is done over a secure channel (e.g., using an out-of-band method), whilst the login is performed over insecure networks. 
 * The Owl protocol ensures that only the registered client with the correct password can pass authentication during the login without revealing the password 
 * to the remote server; upon successful authentication, both the client and the server will establish a common session key for further secure communication.
 ...
 * <li> {@link Owl_Server#authenticationServerResponse(Owl_AuthenticationInitiate, Owl_FinishRegistration)} - use the payload received from the client as well the user credentials stored in the server from user registration, and send the payload generated to the client.</li>
 ...
 * <li> {@link #calculateKeyingMaterial()} - both the server and the client execute this to generate a common key. </li>
 * <li>{@link #initiateKeyConfirmation(BigInteger)} - send the payload to the other participant for key confirmation in this exchange (client/server). This is called by both client and server.</li>
 * <li>{@link validateKeyConfirmation(Owl_KeyConfirmation, BigInteger)} - use the payload received from the other participant to check if the same key has been derived (client/server). This is called by both client and server.</li>
 * </ul>
 * <p>
 ...
 * <p>
 * The explicit key confirmation process is optional, but recommended. 
 * If you do not do explicit key confirmation, then there is no assurance that both client and server are using the same key.
...
public class Owl_Client
{
     ...
     * If you want to handle this detection explicitly, you can perform
     * explicit key confirmation.  See {@link Owl_Client} for details.
     * <p>
